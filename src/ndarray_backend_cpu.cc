...
#define TILE 8
    ...
    void inc_shapenum(std::vector<int32_t> &shapenum, std::vector<int32_t> shape)
    {
      shapenum.back()++;
      for (int32_t i = shapenum.size() - 1; i >= 0; i--)
      {
        if (shapenum[i] >= shape[i])
        {
          shapenum[i] %= shape[i];
          if (i != 0)
            shapenum[i - 1]++;
    ...
    int32_t get_index(std::vector<int32_t> shapenum, std::vector<int32_t> strides)
    {
      int32_t index = 0;
      for (int32_t i = shapenum.size() - 1; i >= 0; i--)
      {
        index += shapenum[i] * strides[i];
      }
      return index;
    }
                 ...
                 std::vector<int32_t> strides, size_t offset)
    {
       ...
       */
      /// BEGIN SOLUTION
      std::vector<int32_t> shapenum;
      int size = 1;
      for (int i = 0; i < shape.size(); i++)
      {
        shapenum.push_back(0);
        size *= shape[i];
      }
      for (int i = 0; i < size; i++)
      {
        int index = get_index(shapenum, strides);
        inc_shapenum(shapenum, shape);
    ...
    void SingleOp(const AlignedArray &a, AlignedArray *out, std::function<scalar_t(scalar_t)> op)
    {
      for (size_t i = 0; i < a.size; i++)
      {
...
PYBIND11_MODULE(ndarray_backend_cpu, m)
{
  ...
  m.def("ewise_maximum", [](const AlignedArray &a, const AlignedArray &b, AlignedArray *out)
        { EwiseOp(a, b, out, static_cast<scalar_t (*)(scalar_t, scalar_t)>(std::fmax)); });
  ...
  m.def("ewise_log", [](const AlignedArray &a, AlignedArray *out)
        { SingleOp(a, out, static_cast<scalar_t (*)(scalar_t)>(std::log)); });
  m.def("ewise_exp", [](const AlignedArray &a, AlignedArray *out)
        { SingleOp(a, out, static_cast<scalar_t (*)(scalar_t)>(std::exp)); });
  m.def("ewise_tanh", [](const AlignedArray &a, AlignedArray *out)
        { SingleOp(a, out, static_cast<scalar_t (*)(scalar_t)>(std::tanh)); });
  ...
  m.def("scalar_maximum", [](const AlignedArray &a, scalar_t val, AlignedArray *out)
        { ScalarOp(a, val, out, static_cast<scalar_t (*)(scalar_t, scalar_t)>(std::fmax)); });
