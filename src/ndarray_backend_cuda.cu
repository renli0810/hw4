...
#define MAX_VEC_SIZE 8
    ...
    __device__ size_t getindex(size_t id, CudaVec shape, CudaVec strides)
    {
      size_t index = 0;
      for (int i = shape.size - 1; i >= 0; i--)
      {
        index += (id % shape.data[i]) * strides.data[i];
        id /= shape.data[i];
      }
      return index;
    }
    ...
    __global__ void
    CompactKernel(const scalar_t *a, scalar_t *out, size_t size, CudaVec shape,
                  CudaVec strides, size_t offset)
    {
      ...
      size_t index = getindex(gid, shape, strides);
      out[gid] = a[index + offset];
    }

    __global__ void
    EwisesetitemKernel(const scalar_t *a, scalar_t *out, size_t size, CudaVec shape,
                       CudaVec strides, size_t offset)
    {
      /**
       * The CUDA kernel for the compact opeation.  This should effectively map a single entry in the
    ...
    __global__ void
    ScalarsetitemKernel(const scalar_t val, scalar_t *out, size_t size, CudaVec shape,
                        CudaVec strides, size_t offset)
    {
      /**
       * The CUDA kernel for the compact opeation.  This should effectively map a single entry in the
    ...
    template <typename Op>
    __global__ void SingleKernel(const scalar_t *a, scalar_t *out, size_t size, Op op)
    {
      size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
      if (gid < size)
        out[gid] = op(a[gid]);
    ...
    struct Tanh
    {
      __device__ scalar_t operator()(scalar_t x) const { return tanh(x); }
    };
    ...
    {
      CudaDims dim = CudaOneDim(out->size);
      SingleKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, Log());
    }

    void EwiseExp(const CudaArray &a, CudaArray *out)
    {
      CudaDims dim = CudaOneDim(out->size);
      SingleKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, Exp());
    }

    void EwiseTanh(const CudaArray &a, CudaArray *out)
    {
      CudaDims dim = CudaOneDim(out->size);
      SingleKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, Tanh());
    }
    /**
    ...
    __global__ void MatmulKernel(const scalar_t *a, scalar_t *b, scalar_t *out, uint32_t m, uint32_t n, uint32_t p)
    {
      ...
      // printf("rows = %d cols = %d blockIdx.x = %d blockIdx.y = %d blockDim.x = %d blockDim.y = %d threadIdx.x = %d threadIdx.y = %d\n", rows, cols, blockIdx.x, blockIdx.y, blockDim.x, blockDim.y, threadIdx.x, threadIdx.y);
      if (rows >= m || cols >= p)
      {
        return;
      }
      ...
      {
        out[rows * p + cols] += a[rows * n + k] * b[k * p + cols];
      }
    }
    ...
    __global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out, size_t size, size_t reduce_size)
    {
      ...
      size_t end = min(start + reduce_size, size);
      if (start < end)
      {
